...
export default class TailwindTheme extends Theme {
    ...
    initializeModal(modal: HTMLElement, options?: ModalOptions): ModalInstance {
        ...
        if (backdrop && !options?.staticBackdrop) {
            (backdrop as HTMLElement).addEventListener('click', (e: Event) => {
    ...
    showModal(modal: HTMLElement): void {
        ...
        setTimeout(() => {
            const backdrop = modal.querySelector('.fixed.inset-0');
            if (backdrop) {
                (backdrop as HTMLElement).classList.remove('opacity-0');
                (backdrop as HTMLElement).classList.add('opacity-100');
            }

            const dialog = modal.querySelector('.' + this.getModalDialogClasses().split(' ')[0]);
            if (dialog) {
                (dialog as HTMLElement).classList.remove('opacity-0', 'translate-y-4', 'sm:translate-y-0', 'sm:scale-95');
                (dialog as HTMLElement).classList.add('opacity-100', 'translate-y-0', 'sm:scale-100');
            }
        }, 10);
    ...
    hideModal(modal: HTMLElement): void {
        ...
        const backdrop = modal.querySelector('.fixed.inset-0');
        if (backdrop) {
            (backdrop as HTMLElement).classList.remove('opacity-100');
            (backdrop as HTMLElement).classList.add('opacity-0');
        }

        const dialog = modal.querySelector('.' + this.getModalDialogClasses().split(' ')[0]);
        if (dialog) {
            (dialog as HTMLElement).classList.remove('opacity-100', 'translate-y-0', 'sm:scale-100');
            (dialog as HTMLElement).classList.add('opacity-0', 'translate-y-4', 'sm:translate-y-0', 'sm:scale-95');
        }
